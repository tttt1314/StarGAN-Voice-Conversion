...
class TestDataset(object):
    ...
    def __init__(self, config):
        ...
        self.mc_files = sorted(glob.glob(join(config.test_data_dir, f'*_{config.src_spk}_*.npy')))
    ...
    with torch.no_grad():
        for idx, wav in enumerate(test_wavs):
            ...
            sf.write(join(config.convert_dir, str(config.resume_iters),
                f'{wav_id}-vctoF0cv-{test_loader.trg_spk}.wav'), wav_transformed, sampling_rate)
            if [True, False][0]:
                wav_cpsyn = world_speech_synthesis(f0=f0_converted, coded_sp=coded_sp, 
                                                ap=ap, fs=sampling_rate, frame_period=frame_period)
                sf.write(join(config.convert_dir, str(config.resume_iters), f'cpsynF0cv-{wav_name}'), wav_cpsyn, sampling_rate)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--src_spk', type=str, default='falset', help = 'target speaker.')
    parser.add_argument('--trg_spk', type=str, default='chest', help = 'target speaker.')

    # Directories.
    parser.add_argument('--train_data_dir', type=str, default='./data/mc/train')
    parser.add_argument('--test_data_dir', type=str, default='./data/mc/convert')
